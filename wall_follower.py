...
try:
   while True:
       ...
       #Cross Product
       v_wall = [np.cos(min_angle), np.sin(min_angle), 0] #x, y, z, this is the vector the robot will be driving to the wall
       up_vector = [0, 0, 1]
       cross = cross_product(v_wall, up_vector) #forward velocity vector


       #Correction Vector
       correction_vector = [0,0,0]
       print("before if statement")
       print("cross index: ", cross[0])
       print("setpoint: ", setpoint)
       if ( cross[0] < setpoint ):  # cross[0] is returning the wrong datatype (0.0003834951982431209+0j). Check the calulation, there should be no j at the end of the result you get
           print("we got this far")
           correction_vector = [setpoint, 0, 0] # correction_vector not previously defined. Redefine this so it is useable if the if statement is not called.
       print("final control")
